...
export class SearchForm extends React.Component {
	...
	formatQuery() {
		...
		this.state.languages.length > 0
			? (s += "languages=" + this.state.languages + "&")
			: "";
				...
				<div className="input-cont">
					...
					<span className="view param">
						...
						<select
							...
							<option value={"login"} onChange={e => this.handleChange(e)}>
								Search By GitHub Username
							</option>
							<option value={"languages"} onChange={e => this.handleChange(e)}>
								Search By Languages
							</option>
							<option value={"roles"} onChange={e => this.handleChange(e)}>
								Search By Roles
							</option>
						</select>
					</span>
					<span className="searchPunc">{":"}</span>
					<span className="view input">
						{this.state.input}
						<input
							type="text"
							className="search"
